...
def mns_request_auth(request):
    ...
    str2sign = "%s\n%s\n%s\n%s" % (request.method, "\n".join(sign_header_list), service_str, request.path)
    str2sign = str2sign.encode('utf-8')

    authorization = headers['Authorization']
    signature = base64.b64decode(authorization)
    cert_str = urllib2.urlopen(base64.b64decode(headers['X-Mns-Signing-Cert-Url'])).read()
...
def mns_request_decorator(fn):
    ...
    def __wrapper(*arg, **kw):
        ...
        try:
            ...
            args['meta']['mns_message_id'] = request.headers['x-mns-message-id']
            fn(args, *arg, **kw)
