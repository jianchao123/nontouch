...
class ActivitiesTimer(object):
    ...
    def change_activity_status(self, mysql_cur):
        ...
        result = mysql_db.query(mysql_cur, coupon_type_sql)
        for row in result:
            pk = row[0]
            ...
            # 活动中状态
                ...
                if cur_time > give_out_end_time:
                    ...
                    volume_result = mysql_db.get(mysql_cur, volume_sql.format(pk))
                    print volume_result
            ...
            if d:
                d["`id`"] = pk
                mysql_db.update(mysql_cur, d, table_name='`coupon_type`')
    ...
    def change_coupe_status(self, mysql_cur):
        ...
        user_coupe_sql = "SELECT uc.`id`,c.`id`,c.`use_begin_time`," \
                         ...
                         "WHERE c.`status`=2"
        mysql_db = db.MysqlDbUtil
        result = mysql_db.query(mysql_cur, user_coupe_sql)
        cur_time = datetime.now()
        for row in result:
            coupon_id = row[1]
            use_end_time = row[3]
            if cur_time > use_end_time:
                mysql_db.update(mysql_cur, {"`id`": coupon_id, "`status`": 4},
                                table_name="`coupon`")
    ...
    def update_coupon_data(self, mysql_cur):
        ...
        for row in result:
            ...
            result = mysql_db.get(mysql_cur, sql)
    ...
    @staticmethod
    def insert_user_coupe(mysql_db, mysql_cur, coupon_ids, user_id,
        ...
        for coupon_id in coupon_ids:
            ...
            mysql_db.insert(mysql_cur, d, table_name="`user_coupe`")
            ...
            mysql_db.update(mysql_cur, d, table_name="`coupon`")
    ...
    def invited_users_give_coupon(self, mysql_cur):
        ...
        while mobile_str:
            ...
            new_user = mysql_db.get(mysql_cur, user_sql.format(new_user))[0]
            invite_user = mysql_db.get(mysql_cur, user_sql.format(invite_user))[0]

            coupon_types = mysql_db.query(mysql_cur, coupon_type_sql)
            for coupon_type in coupon_types:
                ...
                if ct_face_value == Decimal(str(1.5)):
                    result = mysql_db.query(mysql_cur, sql.format(ct_id, 2))
                ...
                if ct_face_value == Decimal(str(0.6)):
                    result = mysql_db.query(mysql_cur, sql.format(ct_id, 4))
                    coupon_ids = [row[0] for row in result]
                ...
                if ct_face_value == Decimal(str(0.3)):
                    result = mysql_db.query(mysql_cur, sql.format(ct_id, 6))
                    coupon_ids = [row[0] for row in result]
                ...
                if new_user_coupon_ids and invite_user_coupon_ids:
                    ActivitiesTimer.insert_user_coupe(
                        mysql_db, mysql_cur, new_user_coupon_ids, new_user,
                        datetime.now(), datetime.now() + timedelta(days=7))
                    ActivitiesTimer.insert_user_coupe(
                        mysql_db, mysql_cur, invite_user_coupon_ids,
...
class IdentitiesData(object):
    ...
    def change_user_identity_status(self, mysql_cur):
        ...
        result = mysql_db.query(mysql_cur, sql)
        cur_time = datetime.now()
        for row in result:
            # 当前时间大于截止时间,修改状态
            if cur_time > row[1]:
                d = {"`id`": row[0], "`status`": 2}
                mysql_db.update(mysql_cur, d, table_name="`passenger_identity`")
...
class StatisticsData(object):
    ...
    def statistics_yesterday_data(self, mysql_cur):
        ...
        # 所有公司
        companies = mysql_db.query(mysql_cur, StatisticsData.company_list_sql)
        for company in companies:
            ...
            mysql_db.insert(mysql_cur, data, table_name="`bc_every_day_data`")
    ...
    def statistics_passenger_flow(self, mysql_cur):
        ...
        data_sql = "SELECT `id` FROM `station_passenger_flow` " \
                   "WHERE `station_id`={}"
        stations = mysql_db.query(mysql_cur, "SELECT `id`,`company_id` FROM `bus_station`")
        for row in stations:
            station_id = row[0]
            company_id = row[1]
            order_numbers = mysql_db.get(mysql_cur, sql.format(station_id))
            result = mysql_db.get(mysql_cur, data_sql.format(station_id))
            ...
            if result:
                data["`id`"] = result[0]
                mysql_db.update(mysql_cur, data, table_name="`station_passenger_flow`")
            else:
                data['`company_id`'] = company_id
                mysql_db.insert(mysql_cur, data, table_name="`station_passenger_flow`")
        ...
        # 线路客流
        bus_routes = mysql_db.query(mysql_cur, "SELECT `id`,`company_id` FROM `bus_route`")
        data_sql = "SELECT `id` FROM `route_passenger_flow` WHERE `route_id`={}"

        for row in bus_routes:
            route_id = row[0]
            company_id = row[1]
            result = mysql_db.get(mysql_cur, route_pf_sql.format(route_id))
            data = {
                "`route_id`": route_id,
                "`number`": result[0] if result else 0
            }
            result = mysql_db.get(mysql_cur, data_sql.format(route_id))
            if result:
                data["`id`"] = result[0]
                mysql_db.update(mysql_cur, data, table_name="`route_passenger_flow`")
            else:
                data['`company_id`'] = company_id
                mysql_db.insert(mysql_cur, data, table_name="`route_passenger_flow`")
...
class EveryFewMinutesExe(object):
    ...
    def every_few_minutes_execute(self, mysql_cur):
        """
        每五分钟执行一次 删除不规则人脸
        :return:
        """
